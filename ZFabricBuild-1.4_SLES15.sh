...
prereq_sles() {
  ...
  sudo zypper --non-interactive addrepo https://download.opensuse.org/repositories/Cloud:Tools/SLE_15_SP1/Cloud:Tools.repo
  sudo zypper refresh
  sudo zypper --non-interactive in git-core gcc make gcc-c++ patterns-sles-apparmor  python3-setuptools python3-devel python3-pip gawk libtool libffi-devel libopenssl-devel bzip2 python3-PyYAML which
  sudo pip install --upgrade pip
  sudo pip install cffi
  sudo pip install docker-compose==1.25.3
  if [ $? != 0 ]; then
    echo -e "\nERROR: Unable to install pre-requisite packages.\n"
    exit 1
  fi
